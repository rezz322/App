            ...
            <div className="max-w-7xl mx-auto">
                ...
                {updateAvailable && (
                    <button 
                        ...
                        className="fixed bottom-8 right-8 z-50 flex items-center bg-orange-500 hover:bg-orange-600 text-white px-4 py-3 rounded-full shadow-2xl transition-all hover:scale-110 active:scale-95 animate-pulse"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 mr-2">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                        </svg>
                        <span className="font-bold text-sm uppercase tracking-wide">Доступне оновлення</span>
                    </button>
                )}
                            ...
                            <table className="min-w-full bg-white">
                                <thead>
                                    <tr className="bg-gray-50 border-b">
                                        {(['id', 'name', 'all_hour', 'field1', 'field1_pm', 'field2', 'field2_pm', 'field3', 'field3_pm', 'date_complited'] as string[]).map((col) => (
                                            <th
                                                key={col}
                                                className="py-3 px-4 text-gray-700 font-semibold text-center text-xs uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors"
                                                onClick={() => {
                                                    if (['id', 'name', 'all_hour', 'field1', 'field2', 'field3', 'date_complited'].includes(col)) {
                                                        handleSort(col as keyof Task);
                                                    }
                                                }}
                                            >
                                                <div className="flex flex-col items-center justify-center space-y-0.5">
                                                    <span>{
                                                        ...
                                                        col === 'name' ? 'НАЗВА' :
                                                        col === 'all_hour' ? 'ГОДИНИ' :
                                                        col === 'field1' ? 'Д1 (РОБОТА)' :
                                                        col === 'field1_pm' ? 'Д1 (ПОЧАТОК/МАТ)' :
                                                        col === 'field2' ? 'Д2 (РОБОТА)' :
                                                        col === 'field2_pm' ? 'Д2 (ПОЧАТОК/МАТ)' :
                                                        col === 'field3' ? 'Д3 (РОБОТА)' :
                                                        col === 'field3_pm' ? 'Д3 (ПОЧАТОК/МАТ)' :
                                                        col === 'date_complited' ? 'ЗАВЕРШЕНО' : col
                                                    }</span>
                                                    {(sortColumn === col || (col === 'date_working' && sortColumn === 'date_working')) && (
                                                        <span className="text-blue-500 text-[10px]">{sortDirection === 'asc' ? '↑' : '↓'}</span>
                                                    )}
                                                </div>
                                            </th>
                                        ))}
                                        <th className="py-3 px-4 text-gray-700 font-semibold text-left text-xs uppercase tracking-wider">Дії</th>
                                    </tr>
                                </thead>
                                <tbody>
