...
import { getNextAvailableTime, isFreeForField } from '../utils/taskCalculations';
...
const AddTaskForm: React.FC<AddTaskFormProps> = ({ tasks, onSubmit }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.field1 < 0 || formData.field2 < 0 || formData.field3 < 0) {
            alert('Дільниці не можуть бути менше 0');
            return;
        }

        if (isFreeForField(tasks, formData.date, formData.field1)) {
            alert('В цей період вже є замовлення на даній дільниці');
            return;
        }
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Наступна доступна: **{getNextAvailableTime(tasks, 'field1_end')}**</p>
                    </div>
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Наступна доступна: **{getNextAvailableTime(tasks, 'field2_end')}**</p>
                    </div>
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Наступна доступна: **{getNextAvailableTime(tasks, 'field3_end')}**</p>
                    </div>
                </div>
